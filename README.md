...
4. Select **Connect to Codex**, restart Codex, and use the local MCP tools.

The CPU choice persists and is shared by every indexing project in the running desktop app. Parallel file jobs use one thread each; OCR and Granite temporarily use the full selected budget so one document can use the available capacity. The separate MCP process reads the same persisted profile when it starts and applies it to search and materialization work.
...
Semantic search is optional. Select **Set up** to install IBM Granite Embedding 311M Multilingual R2. The app explains the Granite and tokenizer terms before download, verifies pinned SHA-256 hashes, supports resume, and falls back to keyword search if the model is unavailable.
...
Package versions are centralized in `Directory.Packages.props`, and lock files are committed. See [docs/DEVELOPMENT.md](docs/DEVELOPMENT.md) for focused smoke tests and [docs/NATIVE-SMOKE.md](docs/NATIVE-SMOKE.md) for native publish checks.
...
The [Windows release workflow](.github/workflows/release-windows.yml) builds the app and MCP sidecar, verifies the payload, creates the Velopack installer/update feed, and publishes the GitHub Release.
