...
6. With adequate memory and a CPU profile that permits at least two workers, queue several PDF/image/Office/email/archive fixtures and verify at least two enter extraction before either finishes. On supported ONNX targets, let several reach OCR and verify OCR itself remains serialized, queued activity names the OCR wait, every file completes without deadlock, and constrained-memory excess work remains queued.
