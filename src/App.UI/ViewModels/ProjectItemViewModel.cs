...
public sealed class ProjectItemViewModel : ViewModelBase
{
    ...
    public Guid Id { get; }
    public ObservableCollection<ProjectErrorInfo> RecentErrors { get; } = [];
    ...
    public void UpdateErrors(IReadOnlyList<ProjectErrorInfo> errors)
    {
        for (var targetIndex = 0; targetIndex < errors.Count; targetIndex++)
        {
            ...
            {
                if (RecentErrors[targetIndex] != incoming) RecentErrors[targetIndex] = incoming;
                continue;
            }
            ...
            {
                RecentErrors.Insert(targetIndex, incoming);
            }
        }
